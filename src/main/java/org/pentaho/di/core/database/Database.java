...
import cn.edu.llxy.dw.dss.database.ProcedureInfoVo;
  ...
  private LoggingObjectInterface parentLoggingObject;
	...
	private void connectUsingClass(String classname, String partitionId) throws KettleDatabaseException
	{
		...
		try 
		{
            ...
            if(databaseMeta.getDatabaseInterface() instanceof Hive2WithKerberosDatabaseMeta){
            	preAuthWithKerberos();
            ...
            }else if (databaseMeta.supportsOptionsInURL())
            {
                if (!Const.isEmpty(username) || !Const.isEmpty(password))
                {
                	// also allow for empty username with given password, in this case username must be given with one space 
            ...
            else
            {
                ...
                if("oracle.jdbc.driver.OracleDriver".equals(classname)){
	...
	private void preAuthWithKerberos() throws Exception{
		Hive2WithKerberosDatabaseMeta kbDatabaseMeta = (Hive2WithKerberosDatabaseMeta)databaseMeta.getDatabaseInterface();
		
		org.apache.hadoop.conf.Configuration conf = new org.apache.hadoop.conf.Configuration();
		System.setProperty("java.security.krb5.conf", kbDatabaseMeta.getKrb5ConfigPath());
		System.setProperty("hadoop.home.dir", kbDatabaseMeta.getHadoopHomeDir());
		
		conf.set("hadoop.security.authentication", "Kerberos");
		conf.setBoolean("fs.hdfs.impl.disable.cache", true);
		UserGroupInformation.setConfiguration(conf);
		try {
			UserGroupInformation.loginUserFromKeytab(kbDatabaseMeta.getUsername(), kbDatabaseMeta.getKeytabPath());
		} catch (IOException e) {
	...
	public Result execStatement(String sql, RowMetaInterface params, Object[] data) throws KettleDatabaseException
	{
		...
		try
		{
            ...
            if (resultSet)
            {
            	...
            	//why not set resultSet to result? kuka add resultSet to result ,result is true 2010-12-31
     ...
     * @throws KettleDatabaseException in case anything goes wrong.
     */
	public ResultSet execStatementResultSet(String sql) throws KettleDatabaseException
	{
		Statement stmt;
		ResultSet rs = null;
		try {
	...
	public RowMetaInterface getQueryFields(String sql, boolean param, RowMetaInterface inform, Object[] data) throws KettleDatabaseException
	{
		...
		// Store in cache!! no use cache by kuka for fixed 
		// AR:http://172.16.9.80:7080/browse/ETL-258
	...
	private ValueMetaInterface getValueFromSQLType(String name, ResultSetMetaData rm, int index, boolean ignoreLength, boolean lazyConversion) throws SQLException
    {
        ...
        if(rm instanceof Hive2WithKerberosDatabaseMeta || rm instanceof HiveResultSetMetaData || rm instanceof HiveResultSetMetaData){
        	signed = false;
        ...
        if(rm instanceof Hive2WithKerberosDatabaseMeta || rm instanceof HiveResultSetMetaData || rm instanceof HiveResultSetMetaData){
        	if(name.indexOf(".") > 0){
        		v.setTableName(name.substring(0, name.indexOf(".")));
        	}
        }else{
        	v.setTableName(rm.getTableName(index));
        }
	...
	public Map<String, Object> getMapRow(ResultSet rs) throws KettleDatabaseException
	{
		return getMapRow(rs, false);
	}
	...
	public Map<String, Object> getMapRow(ResultSet rs, boolean lazyConversion) throws KettleDatabaseException
	{
        if (rowMeta==null)
        {
            ResultSetMetaData rsmd = null;
		...
		return getMapRow(rs, null, rowMeta);
	}
			...
			if (rs.next())
			{
				for (int i=0;i<nrcols;i++)
				{
					...
					case ValueMetaInterface.TYPE_BINARY    : 
                        ...
                        break;
					case 9:{
						xobj = rs.getObject(i+1);
						if(xobj != null){
							Timestamp st = (Timestamp)xobj;
							data[i] = DateUtil.getDate(st, DateUtil.DATE_FORMAT);
						}else{
							data[i] = "";
						}
					}break;
					case ValueMetaInterface.TYPE_DATE      :
						...
						else if (val.getPrecision()!=1 && databaseMeta.supportsTimeStampToDateConversion())
                        {
							...
							xobj = rs.getObject(i+1);
							if(xobj != null){
								java.sql.Date st = (java.sql.Date)xobj;
								data[i] = DateUtil.getDate(st, DateUtil.DATE_FORMAT_SIMPLE);
							}else{
								data[i] = "";
							}
                        }
	...
	public Map<String, Object> getMapRow(ResultSet rs, ResultSetMetaData dummy, RowMetaInterface rowInfo) throws KettleDatabaseException
	{
		try
		{
			int nrcols=rowInfo.size();
			...
			if (rs.next())
			{
				for (int i=0;i<nrcols;i++)
				{
					...
					switch(val.getType())
					{
					case ValueMetaInterface.TYPE_BOOLEAN   : data.put(val.getName(), Boolean.valueOf( rs.getBoolean(i+1))); break;
					case ValueMetaInterface.TYPE_NUMBER    : data.put(val.getName(), new Double( rs.getDouble(i+1))); break;
                    case ValueMetaInterface.TYPE_BIGNUMBER : data.put(val.getName(), rs.getBigDecimal(i+1)); break;
					case ValueMetaInterface.TYPE_INTEGER   : data.put(val.getName(), Long.valueOf( rs.getLong(i+1))); break;
					case ValueMetaInterface.TYPE_STRING    : 
						{
							if (val.isStorageBinaryString()) {
								data.put(val.getName(), rs.getBytes(i+1));
							}
							else {
								data.put(val.getName(), rs.getString(i+1));
							}
						}
						break;
					case ValueMetaInterface.TYPE_BINARY    : 
                        {
                            if (databaseMeta.supportsGetBlob())
                            {
                                ...
                                if (blob!=null)
                                {
                                	data.put(val.getName(), blob.getBytes(1L, (int)blob.length()));
                                }
                                else
                                {
                                	data.put(val.getName(), null);
                                }
                            }
                            else
                            {
					...
					case ValueMetaInterface.TYPE_DATE      :
						...
						{
							// Neoview can not handle getDate / getTimestamp for a Time column
							data.put(val.getName(), rs.getTime(i+1)); break;  // Time is a subclass of java.util.Date, the default date will be 1970-01-01
						}
						else if (val.getPrecision()!=1 && databaseMeta.supportsTimeStampToDateConversion())
                        {
							data.put(val.getName(), rs.getTimestamp(i+1)); break; // Timestamp extends java.util.Date
                        }
                        else 
                        {
                        	data.put(val.getName(), rs.getDate(i+1)); break;
                        }
					default: break;
					}
					if (rs.wasNull()) data.put(val.getName(), null); // null value, it's the default but we want it just to make sure we handle this case too.
				}
			}
			else
			{
	...
	public List<Map<String, Object>> getMapRows(String sql, int limit) throws KettleDatabaseException
	{
	    return getMapRows(sql, limit, null);
	}
	
	public List<Object[]> getRowsWithHeader(String sql, int limit) throws KettleDatabaseException
	{
	    return getRowsWithHeader(sql, limit, null);
	}
	
	public List<Object[]> getRowsWithHeader(String sql, int limit, ProgressMonitorListener monitor) throws KettleDatabaseException
	{
		if (monitor!=null) monitor.setTaskName("Opening query...");
		ResultSet rset = openQuery(sql);
		
        return getRowsWithHeader(rset, limit, monitor);
	}
	
	public List<Object[]> getRowsWithHeader(ResultSet rset, int limit, ProgressMonitorListener monitor) throws KettleDatabaseException
    {
        try
        {
            ...
            int nrcols=rm.getColumnCount();
            Object[] header = new Object[nrcols];
            
            String clmName ;
			for (int x=1;x<=nrcols;x++){
				clmName =  rm.getColumnName(x);
	...
	public List<Map<String, Object>> getMapRows(String sql, int limit, ProgressMonitorListener monitor) throws KettleDatabaseException
	{
		if (monitor!=null) monitor.setTaskName("Opening query...");
		ResultSet rset = openQuery(sql);
		
        return getMapRows(rset, limit, monitor);
	}
	
	public List<Map<String, Object>> getMapRows(ResultSet rset, int limit, ProgressMonitorListener monitor) throws KettleDatabaseException
    {
        try
        {
            ...
            if (rset!=null)
            {
                ...
                while ((limit<=0 || i<limit) && !stop)
                {
                    Map<String, Object> row = getMapRow(rset);
                    if (row!=null)
                    {
                        result.add(row);
                        i++;
	...
	public String[] getTablenames(String schemanamein, String tableNamePattern, boolean includeSchema) throws KettleDatabaseException
	{
		...
		try
		{
			if(databaseMeta.getDatabaseInterface() instanceof MySQLDatabaseMeta || databaseMeta.getDatabaseInterface() instanceof GBaseDatabaseMeta)
				alltables = getDatabaseMetaData().getTables(schemaname, null, tableNamePattern, databaseMeta.getTableTypes() );
			else if(databaseMeta.getDatabaseInterface() instanceof ImpalaDatabaseMeta || databaseMeta.getDatabaseInterface() instanceof MongoDBDatabaseMeta){
				if(tableNamePattern == null)
					tableNamePattern = "%";
				alltables = getDatabaseMetaData().getTables(null, schemaname, tableNamePattern, databaseMeta.getTableTypes() );
			}else if (databaseMeta.getDatabaseInterface() instanceof  HiveDatabaseMeta){
				if(tableNamePattern == null)
					tableNamePattern = "%";
				alltables = getDatabaseMetaData().getTables(schemaname, schemaname, tableNamePattern, null );
			}else{
				alltables = getDatabaseMetaData().getTables(null, schemaname.toUpperCase(), tableNamePattern, databaseMeta.getTableTypes() );
			}
	...
	public String[] getTablenames(String schemanamein, boolean includeSchema) throws KettleDatabaseException
	{
		...
		{
			if(databaseMeta.getDatabaseInterface() instanceof MySQLDatabaseMeta || databaseMeta.getDatabaseInterface() instanceof GBaseDatabaseMeta)
				alltables = getDatabaseMetaData().getTables(schemaname, null, null, databaseMeta.getTableTypes() );
			else if(databaseMeta.getDatabaseInterface() instanceof HiveDatabaseMeta){
				alltables = getDatabaseMetaData().getTables(null, schemaname, null, null);
			}else
	...
	public String[] getViews(String schemanamein, String viewNamePattern, boolean includeSchema) throws KettleDatabaseException
	{
		...
		{
			if(databaseMeta.getDatabaseInterface() instanceof MySQLDatabaseMeta)
				alltables = getDatabaseMetaData().getTables(schemaname, null, viewNamePattern, databaseMeta.getViewTypes() );
			else
				alltables = getDatabaseMetaData().getTables(null, schemaname, viewNamePattern, databaseMeta.getViewTypes() );
			while (alltables.next())
			{
				// due to PDI-743 with ODBC and MS SQL Server the order is changed and try/catch included for safety
				String cat = "";
	...
	public String[] getViews(String schemanamein, boolean includeSchema) throws KettleDatabaseException
	{
		...
		{
			if(databaseMeta.getDatabaseInterface() instanceof MySQLDatabaseMeta || databaseMeta.getDatabaseInterface() instanceof GBaseDatabaseMeta)
				alltables = getDatabaseMetaData().getTables(schemaname, null, null, databaseMeta.getViewTypes() );
			else if(databaseMeta.getDatabaseInterface() instanceof HiveDatabaseMeta){
				alltables = getDatabaseMetaData().getTables(null, schemaname, null, null);
			}else 
	 ...
	 * 
	 */
	public ProcedureInfoVo[] getProcedureColumns(String schemaPattern, String procedureNamePattern) throws KettleDatabaseException {
		ResultSet rs = null;
		try {
			DatabaseMetaData dbmd = getDatabaseMetaData();
			if(procedureNamePattern.indexOf(".") != -1){ // 解决oracle中packages中的存储过程查询不到参数的问题，需要把包名去掉。
				procedureNamePattern = procedureNamePattern.substring(procedureNamePattern.indexOf(".")+1, procedureNamePattern.length());
			}
			rs = dbmd.getProcedureColumns(null, schemaPattern, procedureNamePattern, null);
			List<Object[]> rows = getRows(rs, 0, null);
			ProcedureInfoVo result[] = new ProcedureInfoVo[rows.size()];
			for (int i = 0; i < rows.size(); i++) {
				Object[] row = (Object[]) rows.get(i);
				String procedureName = rowMeta.getString(row, "PROCEDURE_NAME", null);// 列/参数名称
				String columnName = rowMeta.getString(row, "COLUMN_NAME", null);// 列/参数名称
				Long columnType = rowMeta.getInteger(row, "COLUMN_TYPE", null);// 列/参数的种类
				Long dateType = rowMeta.getInteger(row, "DATA_TYPE", null);// 来自 java.sql.Types 的 SQL 类型
				String typeName = rowMeta.getString(row, "TYPE_NAME", null);// SQL 类型名称，对于 UDT 类型，类型名称是完全限定的
				String defaultValue = rowMeta.getString(row, "COLUMN_DEF", null);// 列的默认值，当该值在单引号内时被解释为一个字符串（可以为 null）
				ProcedureInfoVo vo = new ProcedureInfoVo(procedureName, columnName, columnType, dateType, typeName, defaultValue);
				result[i] = vo;
			}
			return result;
		} catch (Exception e) {
