  ...
  private ObjectRevision objectRevision;
    ...
    */
	...
	public DatabaseMeta(String name, String type, String access, String host, String db, String port, String user, String pass,
			String krb5ConfigPath, String hadoopHomeDir,String keytabPath, String principal)
	{
		setValues(name, type, access, host, db, port, user, pass, krb5ConfigPath, hadoopHomeDir, keytabPath, principal);
		addOptions(type,null);
	}
	
	public DatabaseMeta(String name, String type, String access, String host, String db, String port, String user, String pass,String dbParameter)
	{
		setValues(name, type, access, host, db, port, user, pass,"","","","");
		addOptions(type,dbParameter);
	}
	
	/*public DatabaseMeta(EtlManDbResourceVo dbResource)
	{
		setValues(dbResource.getDbName(), dbResource.getDbType(), dbResource.getDbConType(), dbResource.getDbHost(), dbResource.getDbName(), dbResource.getDbPort(), dbResource.getDbUser(), dbResource.getDbPassword());
        addOptions(dbResource);
	}*/
     ...
     *
     */
    public void addOptions(String dbtype,String dbParameter)
    {
    	...
    	PluginInterface teradataPlugin = PluginRegistry.getInstance().getPlugin(DatabasePluginType.class, "TERADATA");
    	
    	String mySQL = mySqlPlugin.getIds()[0];

        addExtraOption(mySQL, "defaultFetchSize", "500");
        addExtraOption(mySQL, "useCursorFetch", "true");
        if("MYSQL".equalsIgnoreCase(dbtype)&&dbParameter!=null && dbParameter.length()>0){
			String[] array = dbParameter.split(";");
			for(int i=0 ;i<array.length;i++){
				String[] array2 = array[i].split("=");
				addExtraOption(mySQL, array2[0], array2[1]);
			}
		}
        ...
        String teradata = teradataPlugin.getIds()[0];
        addExtraOption(teradata, "CHARSET", "ASCII");
        addExtraOption(teradata, "CLIENT_CHARSET", "GBK");
    }
			...
			,String krb5ConfigPath, String hadoopHomeDir,String keytabPath, String principal )
	{
		...
		if(databaseInterface instanceof Hive2WithKerberosDatabaseMeta){
			Hive2WithKerberosDatabaseMeta hiveDbMeta = (Hive2WithKerberosDatabaseMeta)databaseInterface;
			hiveDbMeta.setKeytabPath(keytabPath);
			hiveDbMeta.setKrb5ConfigPath(krb5ConfigPath);
			hiveDbMeta.setHadoopHomeDir(hadoopHomeDir);
			hiveDbMeta.setPrincipal(principal);
		}
	}
