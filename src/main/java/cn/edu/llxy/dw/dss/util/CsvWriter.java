...
public class CsvWriter {
	 ...
	 *                destination stream.
	 */
	public void write(Object xcontent, boolean preserveSpaces)
			throws IOException {
		String content = null;
		if(xcontent == null){
			content = "";
		}else if(xcontent instanceof Double){
			content = df.format(xcontent);
		}else{
			content = xcontent.toString();
		}
	...
	public static void main(String[] args){
		String[] cnt = new String[]{"测试", "中文"};
		CsvWriter wr =new CsvWriter("c:\\ts.csv",',',Charset.forName("utf-8"));
